...
func formatTimeHeader(t time.Time) (ts string, d int) {
...
func (w *fileLogWriter) WriteMsg(when time.Time, msg string, level int) error {
	...
	var fileInfo string
	//file 相关
	if w.LoggerFuncCallDepth > 0 {
		_, file, line, ok := runtime.Caller(w.LoggerFuncCallDepth)
		if !ok {
			file = "???"
			line = 0
		}
		_, filename := path.Split(file)
		fileInfo = fmt.Sprintf("%v:%v", filename, strconv.Itoa(line))
	}
...
const AdapterMyFile = "myfile"

func init() {
	logs.Register(AdapterMyFile, newFileWriter)
}

type MyBeeLogger = logs.BeeLogger
